...
const SignUp = () => {
    ...
    const { SignUpApi, isLoggedIn, LoginChecker } = useContext(ApiContext);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await api.post(SignUpApi, { username, email, password });
            setUsername('');
            setEmail('');
            setPassword('');
        } catch (error) {
    ...
    return (
        <div className='flex flex-col gap-5 items-center h-screen justify-center'>
            {error && <p className='text-red-500 font-bold underline'>Something Wrong</p>}
            <form onSubmit={handleSubmit} className='flex flex-col gap-5 bg-[#8d99ae] h-[60vh] px-5 rounded-2xl justify-center items-center'>
                <div className="input-group">
                    <input value={username} onChange={(e) => setUsername(e.target.value)} required type="text" name="text" autoComplete="off" className="input"></input>
                    <label className="user-label">Username</label>
                </div>
                <div className="input-group">
                    <input value={email} onChange={(e) => setEmail(e.target.value)} required type="email" name="text" autoComplete="off" className="input"></input>
                ...
                <p>Already Have An Account? <span className='font-bold underline'><Link to="/login">Login</Link></span></p>
            </form>
